...
public class JpaBookDao extends JpaGenericDao<Book, Long> implements BookDao {
	...
	public void addBook(Book book) {
		...
		if (bookDb != null) {
			...
			criteriaPublication.add(Example.create(book.getPublication()));
			Publication publication = (Publication) criteriaPublication.uniqueResult();
			if (publication != null) {
				book.setPublication(publication);
			}else{
				Long id = (Long) session.save(publication);
				book.getPublication().setId(id);
			}*/
			
			/*Criteria criteriaAuthor = session.createCriteria(Author.class);
			criteriaAuthor.add(Example.create(book.getAuthor()));
			Author author = (Author) criteriaAuthor.uniqueResult();
			if (publication != null) {
